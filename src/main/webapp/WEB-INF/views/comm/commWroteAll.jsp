...
<div id="wrap">
	<div id="pp_wrap">
		<div class="pp_menu">
			...
			<ul>
				<li class="pp_proposal"><a href="commWroteAll.do?id=${principal.id}">작성 글</a></li>
				<li class="pp_position"><a href="commReplyWrote.do?id=${principal.id}">작성 댓글</a></li>
		...
		<div class="pp_contents">
			...
			<div class="pp_search">
				<form name="searchProposal" action="commWroteAll.do?id=${requestsDto.id}">
				...
				<table class="pp_table">
					...
					<tbody>
						<c:forEach var="requestsInfoDtos" items="${requestsDto.requestsInfoDtos}">
							<tr>
								<!-- 커뮤니티 글 제목으로 이동 -->
								<td><a href="#">${requestsInfoDtos.commSub}</a></td>
								<!-- 커뮤니티 글 상세내용 -->
								<td>${requestsInfoDtos.commText}</td>
								<!-- 커뮤니티 글 조회수 -->
								<td>${requestsInfoDtos.commCount}</td>
								<!-- 해당 글 댓글수 -->
								<td><p class="cnt_highlight">${requestsInfoDtos.replyCount}</p></td>
								<!-- 해당 글 수정일 -->
								<td>${requestsInfoDtos.commUpdate}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
